...
st.markdown('''
    <style>
    body {background-color: #181c25;}
    .stApp {background-color: #181c25;}
    .st-bb {background: #232946; border-radius: 8px; padding: 16px; margin-bottom: 16px;}
    .metric-label {color: #f4f4f4; font-size: 1.1em;}
    .metric-value {font-size: 2em; color: #f7c873;}
    .stButton>button {background-color: #f7c873; color: #181c25; border-radius: 6px;}
    .stSelectbox, .stNumberInput, .stTextInput {background-color: #232946 !important; color: #f4f4f4 !important;}
    label, .st-cb, .st-emotion-cache-1v0mbdj, .st-emotion-cache-1r4qj8v {color: #f4f4f4 !important;}
    </style>
...
def statics():
    ...
    with col1:
        st.markdown(f'<div class="st-bb"><span class="metric-label">Customers Stayed</span><br><span class="metric-value">{retined:.2f}%</span></div>', unsafe_allow_html=True)
    with col2:
        st.markdown(f'<div class="st-bb"><span class="metric-label">Customers Left</span><br><span class="metric-value">{churned:.2f}%</span></div>', unsafe_allow_html=True)
